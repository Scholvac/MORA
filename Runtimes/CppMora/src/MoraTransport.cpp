...
IServer::IServer(Communicator& com, MessageHandler& h)
	:	mCommunicator{com}, mMessageHandler{h}
{
}
...
AbstractConnection::AbstractConnection(const RemoteCommunicator& t, const Options& o)
	: mTarget{ t }, mOptions{ o }
{
}
...
Protocol AbstractConnection::getProtocol() const { return mOptions.protocol; }
...
bool InMsg::handleNextBytes(int bytesRead) {
	...
	if (length < 0 && pos > HEADER_SIZE) { //not yet initialized
		...
		magic = data[o++];
		length = MoraUtils::readInt(buffer, o);
		flags = data[o++];
		numberOfMessages = MoraUtils::readShort(buffer, o);
		messageNumber = MoraUtils::readShort(buffer, o);
...
bool InMsg::requiresAcknowledge(int8* buffer) {
	if ((flags & REQUEST_ACKNOWLEDGE) == REQUEST_ACKNOWLEDGE) {
		...
		buffer[1] = magic;
		MoraUtils::writeShort(buffer, 2, messageNumber);
		return true;
	}
	return false;
}
	...
	: mBuffer((10))
{
	...
	MoraUtils::writeInt(&mBuffer[0], offset, length);
	offset += 4;
	mBuffer[offset++] = 0;//flag - may be changed later by the connection
	MoraUtils::writeShort(&mBuffer[0], offset, numberOfMessages);
	offset += 2;
	MoraUtils::writeShort(&mBuffer[0], offset, messageNumber);
	offset += 2;
