...
namespace De.Sos.MORA.Examples.Serialize {
	...
	public class ExtendedCallbackRPC {
		...
		public static readonly string TYPE_IDENTIFIER = "EXTENDEDCALLBACK";
		...
		public class Proxy : AbstractProxy<IFace>, IFace {
			...
			{
				return ExtendedCallbackRPC.TYPE_IDENTIFIER;
			}
						
			
			private class RemoteCall_ExtendedEcho_FLOAT_ : RemoteVoidMethodCall 
			{
			...
			public Task Async_extendedEcho(float value)
			{
				int methodIndex = 0;
				RemoteMethod method = mMethods[methodIndex];
				if (method == null)
				{
					mMethods[methodIndex] = method = CreateRemoteMethod("ExtendedEcho_FLOAT_");
				}
				RemoteCall_ExtendedEcho_FLOAT_ call = new RemoteCall_ExtendedEcho_FLOAT_(Communicator, method, value );
				return call.invoke();
			}
			
			public void extendedEcho(float value){
				Task _task = Async_extendedEcho(value);
				_task.Wait();
			}
		}
		#endregion
		...
		public class Adapter : AbstractAdapter<IFace> {
			...
			static Adapter() {
				...
				sMethods.Add("ExtendedEcho_FLOAT_", new ExtendedEcho_FLOAT_Invoker());
			}
			...
			private class _getType_Invoker : IMethodInvokation<IFace>
			{
				...
				{
					oStream.Write(ExtendedCallbackRPC.TYPE_IDENTIFIER);
				}
			}
			
			private class ExtendedEcho_FLOAT_Invoker : IMethodInvokation<IFace> {
